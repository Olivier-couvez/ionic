...
export class HomePage {
  ...
  soumettreProposition(proposition: number) {
    if (proposition > this.nbMystere && this.nbTentatives <= this.nbEssais) {
      this.nbTentatives++;
      this.reponse =
        'Trop grand. Essais numéro : ' + this.nbTentatives.toString();
    }
    if (proposition < this.nbMystere && this.nbTentatives <= this.nbEssais) {
      this.nbTentatives++;
      this.reponse =
        'Trop petit. Essais numéro : ' + this.nbTentatives.toString();
    }
    if (proposition === this.nbMystere && this.nbTentatives <= this.nbEssais) {
      this.nbTentatives++;
  ...
  async parametres() {
    const modal = await this.modalCtrl.create({
      ...
      componentProps: {
        mesEssais: this.nbEssais.toString(),
        valmax: this.max.toString(),
      },
    });
    modal.onDidDismiss().then((dataRetuned) => {
      this.param = dataRetuned.data;
    });
